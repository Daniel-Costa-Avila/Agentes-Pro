...
// A faixa do topo é a peça institucional que o Marketing já produziu.
// Caminho, texto alternativo e exibição saem da configuração, para o
// painel poder trocar a arte sem mexer no código.
function bannerHtml(cfg) {
  const arquivo = (cfg.banner_arquivo || '').trim();
  if (cfg.mostrar_banner !== '1' || !arquivo) return '';
  return `<div class="faixa-marca">
    <img src="${escAtributo(arquivo)}" alt="${escAtributo(cfg.banner_alt || '')}">
</div>`;
}
...
function topbarHtml(cfg, menu, atual) {
  return `<header class="topbar">
    <div class="topbar-in">
        <div class="wordmark">
            <img class="wordmark-logo" src="/marca/logo-probel.jpg" alt="${escAtributo(cfg.marca_1 || 'Probel')}">
            <span class="wordmark-risco" aria-hidden="true"></span>
            <span class="wordmark-txt">${esc(cfg.hero_titulo || 'Agentes Pro')}</span>
        </div>
...
function footerHtml(cfg) {
  ...
  return `<footer>
    <div class="footer-in">
        ...
        <img class="footer-logo" src="/marca/logo-lockup.jpg"
             alt="${escAtributo(`${cfg.marca_1 || ''} · ${cfg.marca_2 || ''}`.trim())}">
    </div>
</footer>`;
}
